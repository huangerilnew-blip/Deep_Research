...
class DocumentProcessor:
    ...
    def __init__(
        ...
        # 初始化 MinerUClient（用于 PDF 转 Markdown）
        self.mineru_client = MinerUClient(backend="http-client",base_url=mineru_base_url)
        ...
        for doc_meta in documents:
            try:
                ...
                if file_ext == '.pdf':
                    ...
                    if not md_docs:
                        logger.warning(f"Markdown 文档解析为空: {local_path}")
                        continue
                ...
                logger.error(f"预处理文档失败: {doc_meta.get('title', 'unknown')}, 错误: {e}")
                import traceback
                traceback.print_exc()
                continue
        
        if not docs_for_pipeline:
            logger.warning("没有可处理的文档，返回空结果")
            return []
    ...
    async def _pdf_to_markdown(self, pdf_path: str) -> str:
        ...
        try:
            # 使用 MinerUClient 解析 PDF
            result = await self.mineru_client.parse_pdf(pdf_path)
            
            # 日志记录返回结果的结构（便于调试）
            if result:
                logger.debug(f"MinerU 返回字段: {list(result.keys())}")
            
            # 提取 Markdown 内容
            # 优先使用 'markdown' 字段，如果没有则尝试其他可能的字段名
            markdown_text = None
            
            if isinstance(result, dict):
                # 尝试多种可能的字段名
                for field_name in ['markdown', 'content', 'text', 'md_content']:
                    if field_name in result:
                        markdown_text = result[field_name]
                        logger.debug(f"使用字段 '{field_name}' 提取 Markdown")
                        break
            elif isinstance(result, str):
                # 如果直接返回字符串
                markdown_text = result
                logger.debug("MinerU 直接返回字符串格式")
            
            if markdown_text and isinstance(markdown_text, str):
                logger.info(f"PDF 转换成功: {pdf_path}, 长度: {len(markdown_text)}")
                return markdown_text
            else:
                logger.error(f"PDF 转换失败: 未找到有效的 markdown 内容，返回结果类型: {type(result)}")
                return ""
